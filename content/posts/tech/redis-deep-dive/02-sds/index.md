...
draft: false
summary: "基于 Redis 7.2.14 源码拆解 SDS：它如何保存长度和容量，为什么能 O(1) 取长度，怎样兼容 C 字符串，又如何避免频繁扩容。"
categories: ["tech"]
tags: ["Redis", "SDS", "源码", "C"]
series: ["Redis 深度解析"]
---
...
> 本文基于 **Redis 7.2.14 官方源码**，重点文件是 `src/sds.h` 和 `src/sds.c`。
...
SDS 的名字来自 Simple Dynamic String。Redis 7.2.14 的源码入口很明确：

```text
src/sds.h  -> 类型定义、header 布局、sdslen/sdsavail 这类 inline 函数
src/sds.c  -> 创建、扩容、拼接、释放等实现
```

在 Ubuntu 虚拟机里执行 `make -j$(nproc)` 时，可以在编译输出里看到 Redis 主体源码被逐个编译，
其中就包括：
...
编译完成后，`src/redis-server --version` 显示版本为 `7.2.14`，并且用 6380 端口做过
`PING/SET/GET` 验证。也就是说，下面读的不是孤立文件，而是已经参与构建并被实际运行过的 Redis 源码。

它解决的是 C 字符串在系统软件里几个非常现实的问题：
...
但在 `buf` 前面，Redis 额外放了 header。`src/sds.h` 中可以直接看到：

```c
typedef char *sds;
```

也就是说，SDS 对外就是 `char *`。header 里记录：

- `len`：当前字符串已使用长度；
- `alloc`：当前分配的容量；
- `flags`：header 类型。

所以 `sdslen(s)` 不需要扫描字符串。它读 `s[-1]` 里的 flags，判断 header 类型，
再从 `s` 往前找到对应 header，读出 `len`。
...
|---|---|---|
| `sdshdr5` | 极短字符串 | 只用 flags 的高 5 位存长度，源码注释说明不会真的按结构体访问 |
...
真实源码里这些结构体都带 `__attribute__ ((__packed__))`，避免编译器插入额外 padding。
另外要注意一个细节：`sds.h` 注释写明 `sdshdr5 is never used`，它更多是用来记录布局；
实际读取 type 5 长度时，`sdslen()` 直接从 flags 字节取高 5 位。
...
真实源码位置在 `src/sds.h`，`sdslen()` 是 `static inline`，核心动作就是：
读 `s[-1]` 的 flags，按 `SDS_TYPE_MASK` 分支，返回对应 header 的 `len`。
...
在 Redis 7.2.14 里，公开函数 `sdsMakeRoomFor()` 只是薄封装，真正逻辑在 `_sdsMakeRoomFor()`：

- 先用 `sdsavail(s)` 看剩余空间，够用就直接返回原指针；
- 不够时计算 `len + addlen`；
- 贪婪扩容模式下，如果新长度小于 `SDS_MAX_PREALLOC`，容量翻倍；
- 超过阈值后，每次额外预分配 `SDS_MAX_PREALLOC`；
- 如果 header 类型变化，不能直接 `realloc`，需要新分配、拷贝、释放旧 header。
...
| 函数 | 先看什么 |
|---|---|
| `_sdsnewlen` / `sdsnewlen` | 如何创建 SDS，如何选择 header 类型，如何设置 `len/alloc/flags` |
| `sdslen` | `src/sds.h` 中的 inline 函数，如何 O(1) 读取长度 |
| `sdsavail` | 如何读取剩余容量 |
| `_sdsMakeRoomFor` / `sdsMakeRoomFor` | 扩容策略在哪里发生，贪婪扩容如何工作 |
| `sdscatlen` | 追加内容时如何保证空间足够 |
| `sdsfree` | 释放时为什么要用 `sdsHdrSize(s[-1])` 找到真正的 header 起点 |
...
- SDS 对外是 `char *`，但在 `buf` 前面隐藏 header，保存 `len`、`alloc` 和 `flags`；
- `sdslen()` 能 O(1) 返回长度，因为它读的是 header，不是扫描字符；
- SDS 末尾仍保留 `\0`，所以在合适场景下能兼容 C 字符串；
- 多种 header 类型是为了减少小字符串的内存浪费；其中 `sdshdr5` 要按源码注释理解，实际长度直接编码在 flags 字节里。
