...
从 Redis 视角看，它不是一行“字符串命令”这么简单，而是一段网络输入。大致路径如下：
...
sequenceDiagram
    ...
    Server->>Server: 查命令表，找到 GET
    Server->>Cmd: 执行 getCommand
    Cmd->>DB: 按 key 查找 value
...
第一个字典存真正的数据，第二个字典存过期时间。执行 `GET name` 时，Redis 要做的事大致是：
